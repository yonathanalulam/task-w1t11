...
final class CredentialFileStorageService
{
    ...
    public function storeUploadedFile(UploadedFile $file): array
    {
        ...
        $originalFilename = $originalFilename !== '' ? $originalFilename : 'credential';
        ...
        if (!is_file($absolutePath)) {
            throw new \RuntimeException('Credential file was not persisted.');
        }
    ...
    public function resolveAbsolutePath(string $storagePath): string
    {
        ...
        if ($normalized === '' || str_contains($normalized, '..')) {
            throw new \RuntimeException('Credential storage path is invalid.');
        }

        $absolute = rtrim($this->storageRoot, '/').'/'.$normalized;
        if (!is_file($absolute)) {
            throw new \RuntimeException('Credential file not found in storage.');
        }
